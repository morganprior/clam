...
class DTWMetric(metric.Metric):
    ...
    def __eq__(self, other: "DTWMetric") -> bool:  # type: ignore[override]
        """Check if two `DTWMetric` objects have the same name."""
    ...
    def one_to_one(self, left: numpy.ndarray, right: numpy.ndarray) -> float:
        """`left` and `right` are each a single time-series.

        Compute the dtw distance between the two.
        """
    ...
    def one_to_many(self, left: numpy.ndarray, right: numpy.ndarray) -> numpy.ndarray:
        """`left` is a single time-series and `right` is a 2d array of time-series.

        Compute a 1d array of dtw distances from `left` to each
    ...
    def many_to_many(self, left: numpy.ndarray, right: numpy.ndarray) -> numpy.ndarray:
        """`left` and `right` are both 2d arrays of time-series.

        Each row is a single time-series.Compute a 2d array of distances from
        each time-series in `left` to each instance in `right`.
        """
